...
mod tests {
    ...
    fn test_egraph_from_instruction_supported() {
        ...
        let node =
            ENode::from_instruction(&Instruction::I32Add, &[a, b]).expect("I32Add is supported");
    ...
    fn test_rules_dont_overfire() {
        ...
        assert_ne!(g.find(add), g.find(x), "Add(x, 1) must NOT collapse to x");
    }
    ...
    fn test_i64_add_zero_rule_fires() {
        ...
        assert_eq!(g.find(add), g.find(x), "i64 Add(x, 0) must collapse to x");
    }
