...
REPORT_PATH="${REPORT_PATH:-${REPO_ROOT}/docs/measurements/v1.1.0-corpus-baseline.md}"
...
pct_delta() {
  local new="$1"
  local base="$2"
  # A delta is only meaningful when BOTH operands are integer byte
  # counts. Sentinel strings ("error", "invalid", "timeout", "n/a")
  # must NOT be coerced to 0 by awk — that fabricates a -100% "win"
  # on a run that actually failed or timed out.
...
# --- Emit report -----------------------------------------------------------
{
  echo "# v1.1.0 Corpus Baseline -- LOOM vs wasm-opt -O3"
  echo
